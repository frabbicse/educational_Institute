...
namespace Application.Enroll_Course
{
   public class Detail
    {
        public class Query : IRequest<EnrollCourseDto> { }
             public class Handler : IRequestHandler<Query, EnrollCourseDto>
                {
                    ...
                    public async Task<EnrollCourseDto> Handle(Query request, CancellationToken cancellationToken)
                    {
                //my code goes here
                ...
                return await courses.SingleOrDefaultAsync();
            }
        }
    }
}
